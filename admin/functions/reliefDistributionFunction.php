...
class ReliefDistribution
{
	...
	public function retrieveDistributionList($evac_id, $time)
	{
		...
		$sql ="SELECT d.date_dist, e.package_name, f.operation_name, COUNT(DISTINCT(c.household_id)) as householdnumber FROM attendance as a JOIN resident as b ON a.resident_id = b.resident_id JOIN household as c ON b.household_id = c.household_id JOIN packagedistribution as d ON c.household_id = d.household_id JOIN reliefpackage as e ON d.package_id = e.package_id JOIN reliefoperation as f ON e.operation_id = f.operation_id JOIN evacuationcenter as g ON a.evac_id = g.evac_id WHERE g.evac_id ='".$evac_id."' AND b.house_memship = 'head'";
